...
/**
 * Created by vijjub on 11/12/16.
 */

public class BrowseRoomieAptAdapter extends RecyclerView.Adapter<BrowseRoomieAptAdapter.BrowseRoomieHolder> {

    private List<User> roomieList;
    private Context mContext;

    public BrowseRoomieAptAdapter(List<User> roomieList,Context mcontext) {
        this.roomieList = roomieList;
        this.mContext = mcontext;
    }

    public BrowseRoomieAptAdapter(List<User> roomieList) {
        this.roomieList = roomieList;
    }

    @Override
    public BrowseRoomieHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.browseroomie_apt_list,parent,false);
    ...
    public void onBindViewHolder(BrowseRoomieHolder holder, int position) {
        ...
        Glide.with(mContext).load(user.getProfileImg()).error(R.mipmap.images).override(50,50).centerCrop().skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.SOURCE)
                .into(holder.image);
